...
The frozen `pages` config defines the target page set. `state.json` stores compact `page_plan` ranges plus `target_count`, `active_page`, `completed_count`, and `rendered_count`; large contiguous ranges stay constant-sized. Fresh workspaces advance the accepted and rendered counts together. When an older workspace has accepted pages without page outputs, migration preserves `completed_count`, starts `rendered_count` at zero, and replays those accepted hashes while materializing the new page outputs. The first run also freezes the source PDF size, modification timestamp, and page count. Later source drift returns `config_error` before cached pages can be mixed with a changed input.
...
Dual and `both` modes use a second compact cursor in `output_plan`. After selected pages are translated, each `advance` assembles one source page: selected pages use validated dual shards, while unselected pages use the original page on both sides. Mono-only work publishes immediately after the final target page. Publication checks every required variant and page count before moving any file into `output/`. Public files appear there only at `done`; every intermediate PDF stays under `.pdf_translate/`. All page and output transitions occur inside the foreground `advance` process.
